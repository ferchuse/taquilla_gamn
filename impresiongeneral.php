<?php
	...
	for($x=0;$x<count($textosimp);$x++){
		...
		if(substr(PHP_OS,0,3) != "WIN"){
			...
			//system("copy ".$logo.".TMB lpt1");
			//system("copy nota.txt lpt1: >null:");
			if(file_exists($logo.".TMB")){
				exec('copy '.$logo.'.TMB "\\\\pc-PC\\EPSON TM-T20II Receipt"');
			}
			exec('copy nota.txt "\\\\pc-PC\\EPSON TM-T20II Receipt"');
		}
		if($_GET['copia']==1){
			...
			if(substr(PHP_OS,0,3) != "WIN"){
				...
				exec('copy nota1.txt "\\\\pc-PC\\EPSON TM-T20II Receipt"');
			}
		}
	}
?>